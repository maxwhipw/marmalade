...
// =============================================================================
//
// Compose port of the approved animation suite (internal design mockup
// "animation suite", maintainer 2026-07-04). The static
...
//   LISTENING           lid lifts + tilts with the voice, waves drift in (ALERT)
//   PROCESSING/THINKING gentle sway, eyes drift up, jam bubbles rise, lid taps
...
/**
 * The animated jar. Drop-in for the popup's bottom-left mascot slot —
 * bigger than the old drawable version and drawn live, so the lid, waves,
 * bubbles and blink all move like the approved suite.
 */
@Composable
fun JarMascot(
    ...
    val waveColor = if (isMarmaladeThemeDark()) Color(0xFFFED7AA) else Color(0xFFF97316)
    ...
    Canvas(modifier = modifier.size(size)) {
        ...
        val shiverX = if (state == AssistantState.ERROR) sin(t * .05f) * .7f else 0f
        ...
        scale(u, u, pivot = Offset.Zero) {
            ...
            withTransform({
                ...
                drawJarBody(geo)
                if (state == AssistantState.PROCESSING || state == AssistantState.THINKING) {
                    drawBubbles(geo, t)
                }
...
private fun DrawScope.drawFace(
    ...
    when (state) {
        ...
        AssistantState.LISTENING -> {
            // ALERT: bigger eyes (suite-tuned 5.1×6.0 per the maintainer), parted lips
