...
object GatewayConfigUtils {
    ...
    /**
     * Resolve a scanned QR code or pasted setup code. Handles both:
     * - Raw base64-encoded setup codes (as produced by gateway CLI)
     * - JSON-wrapped codes: { "setupCode": "base64..." }
     *
     * Returns the raw base64 setup code string if valid, or null if unrecognizable.
     */
    fun resolveScannedSetupCode(scannedContent: String): String? {
        val trimmed = scannedContent.trim()
        if (trimmed.isEmpty()) return null
        // Try JSON wrapper first: { "setupCode": "base64..." }
        try {
            val obj = JSONObject(trimmed)
            val inner = obj.optString("setupCode").trim()
            if (inner.isNotEmpty() && decodeGatewaySetupCode(inner) != null) return inner
        } catch (_: Throwable) { /* not JSON, try as raw */ }
        // Try as raw setup code
        return if (decodeGatewaySetupCode(trimmed) != null) trimmed else null
    }
