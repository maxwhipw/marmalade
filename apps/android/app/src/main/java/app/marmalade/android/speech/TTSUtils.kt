...
object TTSUtils {
    ...
    fun setupVoice(tts: TextToSpeech?, speed: Float, languageTag: String? = null) {
        ...
        Log.d(TAG, "Current system locale: $currentLocale")

        // Log engine information
        try {
            val engine = tts?.defaultEngine
            Log.d(TAG, "Using TTS Engine: $engine")
        } catch (e: Exception) {
        ...
        Log.d(TAG, "setLanguage($currentLocale) result=$result")

        if (result == TextToSpeech.LANG_MISSING_DATA || result == TextToSpeech.LANG_NOT_SUPPORTED) {
            // Fallback to English (US) if default fails
            val fallbackResult = tts?.setLanguage(Locale.US)
            Log.d(TAG, "Fallback setLanguage(Locale.US) result=$fallbackResult")
        }
        ...
        try {
            ...
            if (bestVoice != null) {
                tts?.voice = bestVoice
                Log.d(TAG, "Selected voice: ${bestVoice.name} (${bestVoice.locale})")
            }
        } catch (e: Exception) {
